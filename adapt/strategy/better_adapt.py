...
class ParameterizedStrategy(Strategy):
  ...
  def init(self, covered, label, **kwargs):
    ...
    if len(covered) != len(self.neurons):
      print("len(covered):", len(covered), " len(self.neurons):", len(self.neurons))
...
class BetterAdaptiveStrategy(ParameterizedStrategy):

  def __init__(self, network, bound=5, size=50, history=1500, sigma=1):

    super(BetterAdaptiveStrategy, self).__init__(network, bound)
    ...
    self.strategies = [np.random.uniform(
       -self.bound, self.bound, size=EnhancedFeatureMatrix.TOTAL_FEATURES
  ...
  def init(self, covered, label, delta, **kwargs):
    ...
    super(BetterAdaptiveStrategy, self).init(covered=covered, label=label, **kwargs)
  ...
  def update(self, covered, label, distance, **kwargs):
    ...
    super(BetterAdaptiveStrategy, self).update(covered=covered, label=label, **kwargs)
  ...
  def next(self):
    ...
    for _ in range(self.size):
      ...
      """选择特征分布互补的策略"""
      # 按照策略的综合得分计算选择概率
      scores_distribution = scores - np.min(scores)  # 避免负值
      scores_distribution /= np.sum(scores_distribution)  # 得分归一化为概率分布
      # 根据概率选择一对策略，越优秀的策略选到几率越大
