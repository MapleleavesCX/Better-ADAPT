...
#     def covered(self, gradients, internals, **kwargs):

#         covered = []

#         print("internals:", np.concatenate(np.array(internals, dtype=object)).shape)
#         print("gradients:", np.concatenate(np.array(gradients, dtype=object)).shape)

#         if not self._gradients:
#             # 如果是第一次调用，初始化历史记录
#             for layer_output in gradients:
#                 self._gradients.append(np.array([layer_output]))
#                 # 初始情况下，所有神经元都未覆盖
#                 vec = [False] * len(layer_output)
#                 covered.append(np.array(vec))
#         else:
#             # 遍历每一层，计算 SSC 覆盖
#             for index, layer_output in enumerate(gradients):
#                 # 获取历史记录中的符号变化
#                 historical_signs = np.sign(self._gradients[index])[0]
#                 current_signs = np.sign(layer_output)
#                 # 检查符号变化是否满足 SSC 条件
#                 vec = historical_signs != current_signs
#                 covered.append(np.array(vec))
#                 # 更新历史记录
#                 self._gradients[index] = layer_output
...
#         print(f"new_covered: {new_covered}")
